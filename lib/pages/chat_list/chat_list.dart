...
import 'package:zazzychat/pages/chat_list/chat_list_view.dart';
...
import 'package:zazzychat/utils/show_update_snackbar.dart';
...
class ChatListController extends State<ChatList>
  ...
  static const String _serverStoreNamespace = 'im.zazzychat.search.server';
  ...
  void _processIncomingSharedMedia(List<SharedMediaFile> files) {
    ...
    // --- Проверяем: не является ли это внутренним диплинком ZazzyChat ---
    final hasInternalDeepLink = files.any((f) {
      final path = f.path ?? '';
      return path.startsWith('im.zazzychat://@') ||
            path.startsWith('im.zazzychat://chat/');
    });

    if (hasInternalDeepLink) {
      // Просто выходим — ничего не показываем
      return;
    }
    // --- END проверка ---
