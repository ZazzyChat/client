...
[ -n "${ZAZZYCHAT_NEW_GROUP}" ] && {
	# App group IDs
	sed -i "" "s/group.${ZAZZYCHAT_ORIG_GROUP}.app/group.${ZAZZYCHAT_NEW_GROUP}.app/g" "ios/ZazzyChat Share/ZazzyChat Share.entitlements"
	sed -i "" "s/group.${ZAZZYCHAT_ORIG_GROUP}.app/group.${ZAZZYCHAT_NEW_GROUP}.app/g" "ios/Runner/Runner.entitlements"
	sed -i "" "s/group.${ZAZZYCHAT_ORIG_GROUP}.app/group.${ZAZZYCHAT_NEW_GROUP}.app/g" "ios/Runner.xcodeproj/project.pbxproj"
	# Bundle identifiers
	sed -i "" "s/${ZAZZYCHAT_ORIG_GROUP}.app/${ZAZZYCHAT_NEW_GROUP}.app/g" "ios/Runner.xcodeproj/project.pbxproj"
}

[ -n "${ZAZZYCHAT_NEW_TEAM}" ] && {
	# Code signing team
	sed -i "" "s/${ZAZZYCHAT_ORIG_TEAM}/${ZAZZYCHAT_NEW_TEAM}/g" "ios/Runner.xcodeproj/project.pbxproj"
}
cat << EOHELP
...
2. Fix it yourself
    ...
    - Ensure that Xcode created the App ID successfully (for zazzychat.app and zazzychat.app.ZazzyChat-Share)
...
[ -n "${ZAZZYCHAT_INSTALL_IPA}" ] && {
  ...
  cfgutil --foreach install-app "${TMPDIR}/zazzychat.ipa"
  rm -rf "${TMPDIR}"
}
